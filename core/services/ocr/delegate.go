...
func (d Delegate) ServicesForSpec(jb job.Job) (services []job.ServiceCtx, err error) {
	...
	if concreteSpec.IsBootstrapPeer {
		...
		ns := peerWrapper.Config().P2PNetworkingStack()
		if ns != ocrnetworking.NetworkingStackV2 && len(v1BootstrapPeers) < 1 {
			return nil, errors.New("Need at least one v1 bootstrap peer")
		}
		if ns != ocrnetworking.NetworkingStackV1 && len(v2BootstrapPeers) < 1 {
			return nil, errors.New("Need at least one v2 bootstrapper")
		}
